...
// Traffic Statistics (per 1-second window)
struct TrafficStats {
...
extern "C" {
  void wifi_promiscuous_cb(void* buf, wifi_promiscuous_pkt_type_t type) {
    ...
    // Note: this is a heuristic and may not cover all frames
    // Access IP header fields
    ...
    // Need at least 14 bytes for TCP header to read flags and ports
    // Assume ok
    ...
    // Update stats (safe to call from ISR context; keep fast)
    stats.packetCount++;
    stats.tcpCount++;
